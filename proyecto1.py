    ...
    def dibujar(self, ventana):
        ...
        if texto is not None:
            ...
            if self.es_visitado() or self.es_fin() or self.es_inicio() or self.color == VERDE:
                superficie_texto = fuente.render(":v", True, (0, 0, 0))
                ventana.blit(superficie_texto, (
                (self.x + self.ancho) - int(self.ancho / 5), (y - int(self.ancho / 5) - 5) if FILAS > 11 else y))
...
def a_asterico(inicio, final, grid, ventana):
    open_set = [inicio]
    g_score = {}
    f_score = {}

    g_score[inicio] = 0
    f_score[inicio] = h(inicio, final)

    g_lineal = 10
    g_diagonal = 14

    current = inicio
    current.set_text(f"H = {h(current, final)}-G = {g_score[current]}-F = {f_score[current]}")
    ...
    while open_set:
        ...
        print(current.get_numeracion(),
              f"[ h = {h(current, final)}, g = {g_score[current]}, f = {f_score[current]} ] {current.get_pos()}")
        f_score.pop(current)
                    ...
                    if not neighbor_nodo.es_visitado():
                        ...
                        if neighbor_nodo in g_score:
                            if tentative_g_score < g_score[neighbor_nodo]:
                                ...
                                f_score[neighbor_nodo] = h(neighbor_nodo, final) + g_score[neighbor_nodo]
                                neighbor_nodo.set_nodo_dependiente(current)
                                open_set.append(neighbor_nodo)
                                neighbor_nodo.set_text(
                                    f"H = {h(neighbor_nodo, final)}-G = {g_score[neighbor_nodo]}-F = {f_score[neighbor_nodo]}")
                        else:
                            g_score[neighbor_nodo] = tentative_g_score
                            f_score[neighbor_nodo] = h(neighbor_nodo, final) + g_score[neighbor_nodo]
                            neighbor_nodo.set_nodo_dependiente(current)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_i:
                    if inicio and fin:
                        a_asterico(inicio, fin, grid, ventana)
